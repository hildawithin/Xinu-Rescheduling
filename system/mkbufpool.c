	...
	)
{
	...
	buf = (char *)getmem( numbufs * (bufsiz+sizeof(bpid32)) ); /* sizeof(bpid32) 4 bytes more to store meta data*/
	if ((int32)buf == SYSERR) {
		restore(mask);
		return (bpid32)SYSERR;
	}
	...
	if ( (bpptr->bpsem = semcreate(numbufs)) == SYSERR) {
		...
		nbpools--;
		restore(mask);
		return (bpid32)SYSERR; /* in linux and unix system, relocation of memory id needed; Xinu wasting resources */
	}
